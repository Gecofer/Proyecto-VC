...
def mosaicoN(lista_imagenes):
    ...
    for i in range (len(lista_imagenes)-1):
        ...
        keypoints_imagen1 = np.float32([
            keypoints[i][j.queryIdx]
            for j in matches
        ])

        keypoints_imagen2 = np.float32([
            keypoints[i+1][j.trainIdx]
            for j in matches
        ])

        # Calcular la homografía entre los dos conjuntos de puntos
        h, status = cv2.findHomography(
            srcPoints=keypoints_imagen1,
            dstPoints=keypoints_imagen2,
            method=cv2.RANSAC,
    ...
    tras = np.matrix([
        [1, 0, lista_imagenes[centro].shape[1]],
    ...
    for i in range(centro + 1, len(lista_imagenes)):
        ...
        derecha = np.matrix(np.eye(3, dtype=np.float32))
